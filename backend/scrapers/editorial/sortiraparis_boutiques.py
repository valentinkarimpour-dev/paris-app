...
def _fetch_jina(url: str) -> str:
    try:
        ...
        logger.debug("[sortiraparis_boutiques] Jina : %s", e)
    return ""
...
class SortirAParisBoutiques(BaseScraper):
    name = "sortiraparis_boutiques"
    base_url = "https://www.sortiraparis.com"

    INDEX_URL = "https://www.sortiraparis.com/articles/tag/boutique-ephemere"
    MAX_ARTICLES = 5
    CUTOFF_DAYS = 30
    ...
    def scrape(self) -> list[dict]:
        ...
        if not index_content:
            logger.warning("[sortiraparis_boutiques] index vide")
            return []
        ...
        logger.info(
            "[sortiraparis_boutiques] %d articles trouvés", len(article_urls)
        )
        ...
        for url in article_urls:
            ...
            if not page_text:
                logger.warning(
                    "[sortiraparis_boutiques] article vide : %s", url
                )
                continue
            ...
            if not data.get("date_debut"):
                logger.debug(
                    "[sortiraparis_boutiques] skipped (pas de date_debut) : %s",
                    data.get("titre")
                )
                continue
            if data["date_debut"] < cutoff:
                logger.debug(
                    "[sortiraparis_boutiques] skipped (date ancienne %s) : %s",
            ...
            if date_debut_str and date_fin_str:
                try:
                    ...
                    if (d2 - d1).days > 90:
                        logger.debug(
                            "[sortiraparis_boutiques] skipped (durée > 90j) : %s",
                            data.get("titre")
                        )
                        continue
                except ValueError:
            ...
            if not lat:
                logger.debug(
                    "[sortiraparis_boutiques] skipped pas de lieu : %s",
                    data.get("titre")
                )
                continue
        ...
        logger.info(
            "[sortiraparis_boutiques] %d events extraits", len(results)
        )
        return results
