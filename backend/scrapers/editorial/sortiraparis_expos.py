...
ARTICLE_PATTERN = re.compile(
    r"sortiraparis\.com/(?:arts-culture|actualites)/[^/]+/articles/\d+"
)
...
def _fetch_jina(url: str) -> str:
    try:
        ...
        logger.debug("[sortiraparis_expos] Jina : %s", e)
    return ""
...
class SortirAParisExpos(BaseScraper):
    name = "sortiraparis_expos"
    base_url = "https://www.sortiraparis.com"

    INDEX_URL = "https://www.sortiraparis.com/arts-culture/exposition"
    MAX_ARTICLES = 15
    CUTOFF_DAYS = 30
    ...
    def scrape(self) -> list[dict]:
        ...
        if not index_content:
            logger.warning("[sortiraparis_expos] index vide")
            return []
        ...
        logger.info(
            "[sortiraparis_expos] %d articles trouvés", len(article_urls)
        )
        ...
        for url in article_urls:
            ...
            if not page_text:
                logger.warning(
                    "[sortiraparis_expos] article vide : %s", url
                )
                continue

            if self._is_list_article(url, page_text):
                logger.debug(
                    "[sortiraparis_expos] skipped article-liste : %s", url
                )
                continue
            ...
            if not data.get("date_debut"):
                logger.debug(
                    "[sortiraparis_expos] skipped (pas de date_debut) : %s",
                    data.get("titre")
                )
                continue
            if data["date_debut"] < cutoff:
                logger.debug(
                    "[sortiraparis_expos] skipped (date ancienne %s) : %s",
                    data["date_debut"], data.get("titre")
                )
                continue

            # Filtre durée > 18 mois (548 jours)
            ...
            if date_debut_str and date_fin_str:
                try:
                    ...
                    if (d2 - d1).days > 548:
                        logger.debug(
                            "[sortiraparis_expos] skipped durée > 18 mois : %s",
                            data.get("titre")
                        )
                        continue
                except ValueError:
            ...
            if not lat:
                logger.debug(
                    "[sortiraparis_expos] skipped pas de lieu : %s",
                    data.get("titre")
                )
                continue
        ...
        logger.info(
            "[sortiraparis_expos] %d events extraits", len(results)
        )
        return results
