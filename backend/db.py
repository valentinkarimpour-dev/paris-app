    ...
    with _conn() as conn:
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS galeries (
                ...
                website      TEXT,
                adresse      TEXT,
                gallery_type TEXT DEFAULT 'inconnu',
                type_source  TEXT,
                scraped_at   TEXT
            )
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_gal_osm ON galeries(osm_id)")
        ...
        conn.execute("""
            CREATE TABLE IF NOT EXISTS gallery_events (
                ...
                gallery_lat  REAL,
                gallery_lng  REAL,
                expo_titre   TEXT NOT NULL,
                expo_url     TEXT,
                date_debut   TEXT,
                date_fin     TEXT,
                confidence   TEXT DEFAULT 'low',
                scraped_at   TEXT,
                UNIQUE(gallery_id, expo_titre)
            )
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_ge_gallery ON gallery_events(gallery_id)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_ge_debut   ON gallery_events(date_debut)")

        conn.execute("""
            CREATE TABLE IF NOT EXISTS gallery_scrape_log (
                gallery_name TEXT PRIMARY KEY,
                scraped_at   TEXT NOT NULL,
                expo_count   INTEGER NOT NULL DEFAULT 0
            )
        """)
...
def upsert_gallery(g: dict) -> int:
    """Insère ou met à jour une galerie. Retourne son id."""
    now = datetime.now().isoformat(timespec="seconds")
    with _conn() as conn:
        conn.execute("""
            INSERT INTO galeries (osm_id, nom, lat, lng, website, adresse, gallery_type, type_source, scraped_at)
            VALUES (:osm_id, :nom, :lat, :lng, :website, :adresse, :gallery_type, :type_source, :scraped_at)
        ...
        """, {**g, "scraped_at": now})
        conn.commit()
        row = conn.execute("SELECT id FROM galeries WHERE osm_id=?", (g["osm_id"],)).fetchone()
    return row[0]


def insert_gallery_event(ev: dict) -> bool:
    now = datetime.now().isoformat(timespec="seconds")
    ev.setdefault("scraped_at", now)
    try:
        with _conn() as conn:
            cur = conn.execute("""
                INSERT OR IGNORE INTO gallery_events
                    (gallery_id, gallery_name, gallery_lat, gallery_lng,
                     expo_titre, expo_url, date_debut, date_fin, confidence, scraped_at)
                VALUES
                    (:gallery_id, :gallery_name, :gallery_lat, :gallery_lng,
                     :expo_titre, :expo_url, :date_debut, :date_fin, :confidence, :scraped_at)
            """, ev)
        ...
        logger.exception("insert_gallery_event failed pour %s", ev.get("expo_titre"))
        return False


def log_gallery_scrape(gallery_name: str, expo_count: int) -> None:
    now = datetime.now().isoformat(timespec="seconds")
    with _conn() as conn:
        conn.execute("""
            INSERT INTO gallery_scrape_log (gallery_name, scraped_at, expo_count)
            VALUES (?, ?, ?)
            ON CONFLICT(gallery_name) DO UPDATE SET
                scraped_at  = excluded.scraped_at,
                expo_count  = MAX(gallery_scrape_log.expo_count, excluded.expo_count)
        """, (gallery_name, now, expo_count))
        conn.commit()


def get_galleries_ever_parsed() -> list[str]:
    with _conn() as conn:
        rows = conn.execute(
            "SELECT gallery_name FROM gallery_scrape_log WHERE expo_count > 0"
        ).fetchall()
    return [r[0] for r in rows]


def get_recent_gallery_expos(
    lat: float | None = None,
    lng: float | None = None,
    ...
    with _conn() as conn:
        rows = conn.execute("""
            ...
            WHERE confidence = :confidence
              ...
              AND date_debut >= :cutoff
              AND (date_fin IS NULL OR date_fin >= :today)
        """, {"confidence": confidence, "today": today, "cutoff": cutoff}).fetchall()

    result: dict[str, list[dict]] = {}
    for row in rows:
        ev = dict(row)
        if lat is not None and lng is not None and radius_m is not None:
            if ev.get("gallery_lat") and ev.get("gallery_lng"):
                if _haversine(lat, lng, ev["gallery_lat"], ev["gallery_lng"]) > radius_m:
                    continue
        name = ev["gallery_name"]
